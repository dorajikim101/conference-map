# Round 2 개발 핸드오프 - Claude Code 작업

## 중요: 작업 디렉토리
- **반드시 이 디렉토리에서 작업:** `/home/node/.openclaw/workspace-qwenfree/conference-map`
- git remote: `https://github.com/dorajikim101/conference-map`
- 브랜치: `main`

## 작업 개요
기존 1차 스캐폴딩 코드를 개선하고, 실제 2026년 행사 데이터로 교체. UI를 한국어로 변경.

## 핵심 변경사항

### 1. 언어 변경 (최우선)
- **모든 UI 텍스트를 한국어로 변경**
- 이벤트 이름과 기업명, 도시명은 영어 그대로 유지
- 버튼, 라벨, 헤더, 설명, 날짜 포맷 등은 한국어로

### 2. 이벤트 데이터 교체
기존 4개 더미 이벤트를 아래 실제 데이터로 교체. **시간순 정렬** 필수.

현재 날짜: 2026년 4월 28일

#### 이벤트 1: Bitcoin 2026 (진행 중)
```typescript
{
  id: "bitcoin-2026",
  name: "Bitcoin 2026",
  ...
  date: "2026-04-27",  // Apr 27-29, 2026
  endDate: "2026-04-29",
  days: 3,
  venue: "The Venetian",
  tier: "S",
  ...
  summary: {
    features: [
      "세계 최대 규모 비트코인 컨퍼런스",
      "채굴, 에너지, 정책, 기업 트랙 등 다양한 트랙 구성",
      "5,000+ 기업, 500+ 연사 참여"
    ],
    difference: [
      "비트코인 단일 자산 집중 - 다른 프로토콜 없음",
      "채굴 및 에너지 인프라에 특화",
      "정책 및 국가 채택 논의가 핵심"
    ],
    pros: [
      "비트코인 생태계 전체를 아우르는 최고의 자리",
      "채굴/에너지/정책 네트워킹 기회 풍부",
      "글로벌 미디어 관심도 높음"
    ],
    risks: [
      "비트코인 외 프로토콜 관련성 낮음",
      "VIP/Whale Pass가 접근성 제한할 수 있음",
      "라스베이거스 호텔 가격 변동 큼"
    ],
    expectations: [
      "채굴 인프라 파트너십 논의",
...
```typescript
{
  ...
  status: "cancelled",  // ⚠️ 취소됨 - 중동 전쟁으로 2027년으로 연기
  ...
  summary: {
    ...
    risks: [
      "2026년 행사 취소 - 2027년으로 연기",
      "프리미엄 패스 업셀로 접근성 제한 가능",
      "호텔 가격 빠르게 변동"
    ],
    expectations: [
      "2027년 재개 시 참고용",
...
```typescript
{
  ...
  summary: {
    features: [
      "CoinDesk 주최 북미 최대 크립토 컨퍼런스",
      "크립토·금융·테크·정책 리더 20,000+ 집결",
      "마이애미 비치 컨벤션 센터"
    ],
    difference: [
      "AI 에이전트 상거래(Agentic Commerce) 트랙 신설",
      "기관 통합(Institutional Integration)에 집중",
      "코인데스크 저널리즘 기반 독립적 프로그래밍"
    ],
    pros: [
      "미주 최대 규모 크립토 집회",
      "투자 유치, BD, 정책 네트워킹 기회 풍부",
      "다양한 트랙으로 포괄적 시야 확보"
    ],
    risks: [
      "프리미엄 패스 가격 높음",
      "마이애미 호텔 예약 경쟁 치열",
      "행사 규모가 커서 타겟팅 어려울 수 있음"
    ],
    expectations: [
      "기관 채택 동향 파악",
      "AI+크립토 교차 분야 탐색",
...
#### 이벤트 4: Digital Assets Forum Abu Dhabi
```typescript
{
  id: "daf-abu-dhabi",
...
#### 이벤트 5: Crypto Valley Conference (보너스)
```typescript
{
  ...
  summary: {
    features: [
      "스위스 '크립토 밸리' 기반 기술 컨퍼런스",
      "프로토콜 엔지니어, 연구자, 규제자 집중",
      "심층 기술 대화 중심"
    ],
    difference: [
      "홍보보다 기술 심도에 집중",
...
### 3. 데이터 구조 변경사항
`EventData` 타입에 아래 필드 추가:
```typescript
status: "upcoming" | "live" | "completed" | "cancelled";  // 행사 상태
endDate?: string;  // 종료일
venue?: string;  // 행사 장소
cancelReason?: string;  // 취소 사유 (status가 cancelled일 때)
```

### 4. 취소 행사 UI 표시
- 리스트에서 취소된 행사는 **빨간 뱃지 "취소"** 표시
- 클릭해서 우측 상세 페이지 열면:
  - 상단에 눈에 띄는 **"이 행사는 취소되었습니다"** 배너
  - 취소 사유 표시
  - 아카이브/과거 데이터는 회색으로 표시하되 여전히 볼 수 있음
...
### 7. 날짜 포맷
- 한국어 날짜 표시: "2026년 4월 27일 ~ 4월 29일"

## UI 텍스트 한국어 변경 예시
| English | 한국어 |
|---------|--------|
| Upcoming Events | 다가오는 행사 |
| Event Details | 행사 상세 |
| Key Summary | 핵심 요약 |
| Features | 특징 |
| Difference | 차별점 |
...
| Budget | 예산 |
| Side Event Trend | 사이드 이벤트 추이 |
| KPI | 핵심 지표 |
| Actions | 추천 액션 |
| Companies | 관심 기업 |
| Meeting Fit | 미팅 적합도 |
| Archive Lift | 아카이브 성장 |
| Update Cadence | 업데이트 주기 |
| Tier | 티어 |
| Select an event | 행사를 선택하세요 |
...
## 작업 순서
1. `src/lib/events.ts` - 타입 추가 + 데이터 교체
2. `src/lib/format.ts` - 한국어 날짜 포맷 추가
3. `src/components/` - 모든 컴포넌트 UI 텍스트 한국어 변경
4. `src/components/EventList.tsx` - 시간순 정렬 + 상태 뱃지 (LIVE/취소)
5. `src/components/EventDetail.tsx` - 취소 배너 + 상태 표시
6. `src/components/StatusBadge.tsx` - 신규: 상태 뱃지 컴포넌트
7. 전체 빌드 확인 (`npm run build`)

## Git 작업
1. `feature/round2-korean-data` 브랜치 생성
2. 커밋 메시지: `feat: 실제 2026 행사 데이터 적용 + 한국어 UI + 취소/라이브 상태 표시`
3. `main`에 머지 후 push
